...
4. [Image Management](#️-image-management)
...
| `docker ps --format "table {{.Name}} {{.Status}}"` | Custom output              | Clean formatted view                                        |
...
| `docker exec -it container cmd` | Interactive mode        | `docker exec -it app bash` → Enter shell          |
...
| `docker cp src dest`            | Copy files to container | `docker cp config.yaml app:/etc/` → Add config    |
...
| `docker run --mount type=volume,src=vol,dst=path image` | Modern mount   | Explicit, recommended syntax                              |
...
```bash
docker ps --filter "network=host"        # Find containers on host network
